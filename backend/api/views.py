...
def get_change_requests_for_patient(request, patient_id):
    if request.method == 'GET':
        try:
            # Première requête pour récupérer les ID des nouveaux médecins uniques pour chaque patient
            ...
            # Deuxième requête pour récupérer les demandes de changement de médecin correspondantes
            change_requests = DoctorChangeRequest.objects.filter(id__in=subquery)
            
            # Convertir les résultats en JSON avec l'email du nouveau médecin
            data = [{'id': request.id, 
